...
/**
 * 추억 사진 S3 저장소.
 * 프로필 이미지 저장소(S3ProfileImageStorage)와 동일한 검증/업로드 방식을 따르되,
 * course 도메인 전용 키 프리픽스(memory-photos/{courseId}/)와 에러 코드를 사용한다.
 */
@Component
@RequiredArgsConstructor
public class S3MemoryPhotoStorage implements MemoryPhotoStorage {
    ...
    @Override
    public String upload(Long courseId, MultipartFile image) {
        ImageType imageType = validate(image);
        S3Client client = client();
        String key = "memory-photos/" + courseId + "/" + UUID.randomUUID() + imageType.extension;
        ...
        try (InputStream inputStream = image.getInputStream()) {
            ...
            throw new ProjectException(CourseErrorCode.MEMORY_PHOTO_UPLOAD_FAILED);
        }
    }
    ...
    // content-type 헤더만으로는 위조가 가능하므로, 매직 넘버(시그니처)와 실제 디코딩까지 확인한다.
    private ImageType validate(MultipartFile image) {
        if (image == null || image.isEmpty()) {
            throw new ProjectException(CourseErrorCode.INVALID_MEMORY_PHOTO_FORMAT);
        }
        if (image.getSize() > MAX_IMAGE_BYTES) {
            throw new ProjectException(CourseErrorCode.MEMORY_PHOTO_TOO_LARGE);
        }
    ...
    private S3Client client() {
        if (!StringUtils.hasText(bucket) || !StringUtils.hasText(region)) {
            throw new ProjectException(CourseErrorCode.MEMORY_PHOTO_STORAGE_NOT_CONFIGURED);
        }
